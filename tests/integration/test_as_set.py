...
@pytest.mark.asyncio
async def test_as_set_expand_sync_success():
    # Mock cache
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get("/api/v1/as-set/expand?target=AS-HURRICANE")
...
@pytest.mark.asyncio
async def test_as_set_expand_cache_hit():
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get("/api/v1/as-set/expand?target=AS-TEST")
            assert response.status_code == 200
            data = response.json()
            assert data["status"] == "completed"
...
@pytest.mark.asyncio
async def test_as_set_expand_non_json_format():
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get("/api/v1/as-set/expand?target=AS-TEST&format=cisco")
            assert response.status_code == 200
            data = response.json()
            assert data["status"] == "completed"
...
@pytest.mark.asyncio
async def test_as_set_expand_async_timeout():
    # Mock cache
    mock_cache = AsyncMock()
    mock_cache.get.return_value = None
    mock_cache.generate_key.return_value = "test-cache-key"

    # Mock bgpq4 client that times out
    mock_client = AsyncMock()
