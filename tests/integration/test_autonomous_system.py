...
@pytest.mark.asyncio
async def test_autonomous_system_prefixes():
    # Mock cache
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get("/api/v1/autonomous-system/prefixes?target=AS15169")
...
@pytest.mark.asyncio
async def test_autonomous_system_prefixes_cache_hit():
...
@pytest.mark.asyncio
async def test_autonomous_system_prefixes_non_json_format():
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.get(
                "/api/v1/autonomous-system/prefixes?target=AS15169&format=cisco"
            )
            assert response.status_code == 200
            data = response.json()
            assert data["status"] == "completed"
...
@pytest.mark.asyncio
async def test_autonomous_system_prefixes_async_timeout():
